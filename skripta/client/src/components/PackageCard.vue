<template>
  <div class="group relative flex flex-col rounded-2xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark overflow-hidden transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-none hover:border-primary/40">
    <button
      class="absolute top-2.5 right-2.5 z-10 w-8 h-8 inline-flex items-center justify-center rounded-lg bg-white/90 dark:bg-surface-dark/90 backdrop-blur text-slate-500 hover:text-slate-800 dark:hover:text-white opacity-0 group-hover:opacity-100 focus:opacity-100 transition"
    ...
    <Transition name="fade">
      <div v-if="menuOpen" class="absolute top-11 right-2.5 z-20 w-44 rounded-xl border border-slate-200 dark:border-border-dark bg-white dark:bg-surface-dark shadow-lg py-1.5" @click.stop.prevent>
        ...
        <button class="w-full flex items-center gap-2 text-left px-3.5 py-2 text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition disabled:opacity-40" :disabled="pkg.status && pkg.status !== 'completed'" @click="duplicate">
          <DocumentDuplicateIcon class="w-4 h-4" /> Duplicate
        </button>
        <button class="w-full flex items-center gap-2 text-left px-3.5 py-2 text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition disabled:opacity-40" :disabled="pkg.status && pkg.status !== 'completed'" @click="exportAs('md')">
          <ArrowDownTrayIcon class="w-4 h-4" /> Export Markdown
        </button>
        <button class="w-full flex items-center gap-2 text-left px-3.5 py-2 text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition disabled:opacity-40" :disabled="pkg.status && pkg.status !== 'completed'" @click="exportAs('json')">
          <ArrowDownTrayIcon class="w-4 h-4" /> Export JSON
        </button>
    ...
    <RouterLink :to="`/package/${pkg._id}`" class="flex flex-col flex-1" @click="renaming ? $event.preventDefault() : null">
      ...
      <div class="flex flex-col flex-1 p-5">
        ...
        <span v-if="pkg.status === 'failed'" class="badge badge-danger self-start mb-1.5">Generation failed</span>
        <span v-else-if="pkg.status && pkg.status !== 'completed'" class="badge badge-primary self-start mb-1.5">
          <ArrowPathIcon class="w-3 h-3 mr-1 animate-spin" /> Generating… {{ pkg.progress || 0 }}%
        </span>
    ...
    <Modal :open="confirmDelete" title="Delete this study package?" confirm-label="Delete" @close="confirmDelete = false" @confirm="doDelete">
