<template>
  <RouterLink
    :to="`/package/${pkg._id}`"
    class="group flex flex-col rounded-2xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark p-5 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-none hover:border-primary/40"
  >
...
import { QuestionMarkCircleIcon, Squares2X2Icon } from "@heroicons/vue/24/outline";

defineProps({ pkg: { type: Object, required: true } });
