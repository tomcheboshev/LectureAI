    ...
    <div class="w-full max-w-sm text-center">
      ...
      <div class="rounded-2xl border border-slate-200 dark:border-border-dark bg-white dark:bg-surface-dark p-6 sm:p-8 shadow-xl shadow-slate-200/40 dark:shadow-none">
        ...
        <h1 class="font-display font-bold text-xl text-slate-900 dark:text-white mb-1.5">
          {{ status === "loading" ? t("auth.verifyEmailChange.verifying") : status === "success" ? t("auth.verifyEmailChange.success") : t("auth.verifyEmailChange.failed") }}
        </h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-5">{{ message }}</p>
        <RouterLink :to="auth.isAuthenticated ? '/settings' : '/login'" class="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white hover:bg-primary-hover transition">
          {{ auth.isAuthenticated ? t("auth.verifyEmailChange.goToSettings") : t("auth.verifyEmail.logIn") }}
        </RouterLink>
      </div>
    </div>
  </div>
...
onMounted(async () => {
  try {
    const res = await api.verifyEmailChange({ token: route.params.token });
